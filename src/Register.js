...
function Register() {
  ...
  const handleChange_role = (e) => {
    regitserType = { ...user };
    console.log(user);
    regitserType[e.target.id].id = e.target.value;
    //set the form values
    setUser(regitserType);
  };
  ...
  const register = (values) => {
   // debugger;
    ...
    const mobile_no_regex = /(\+)?(91)?( )?[789]\d{9}/g;
    //regular expression for validating name
    const name_regex = /^[a-zA-Z ]{2,40}$/;
    //regular expression for validating password

    const address_regex = /^[a-zA-Z0-9 ]{2,40}$/;

    const pincode_regex =    /^[1-9][0-9]{5}$/;

    const user_type_id_regex= /(\+)?(91)?( )?[789]\d{9}/g;
        ...
        <table >
          <tbody>
            <tr>
              <td>Name:</td>
              <td>
                <input value={user.name} type="text" id="name" onChange={handleChange}/>
              </td>
              <td>{error.name}</td>
            </tr>
            <tr>
           ...
           <input type="radio" value="M" id="gender" name="gender"/> Male &nbsp;
            ...
            <tr>
              <td>Register as:</td>
              <td>
                <select style={{width:'190px'}} value={user.user_type_id} id="user_type_id" onChange={handleChange_role} name="user_type_id" required>
